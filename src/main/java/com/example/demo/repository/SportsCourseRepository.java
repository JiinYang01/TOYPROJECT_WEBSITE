...
public interface SportsCourseRepository extends JpaRepository<SportsCourse, Long> {
    ...
    @Query("SELECT sc FROM SportsCourse sc WHERE sc.rowNum = 1 AND sc.category.categoryId = :categoryId ORDER BY sc.crseNum DESC")
    ...
    @Query("SELECT sc FROM SportsCourse sc WHERE sc.rowNum = 1 AND sc.category.categoryId = :categoryId AND sc.ctprvnNm = :ctprvnNm ORDER BY sc.crseNum DESC")
    ...
    @Query("SELECT sc FROM SportsCourse sc WHERE sc.rowNum = 1 AND sc.category.categoryId = :categoryId AND sc.ctprvnNm = :ctprvnNm AND sc.signguNm = :signguNm ORDER BY sc.crseNum DESC")
    ...
    @Query("SELECT sc FROM SportsCourse sc WHERE sc.rowNum = 1 AND sc.ctprvnNm = :ctprvnNm ORDER BY sc.crseNum DESC")
